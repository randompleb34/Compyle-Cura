...
# File Name         : AScriptWriter.py
# Description       : Plugin to enable Cura 5.8 and above to write .ascript files for Aerotech Automation 1
...
# ------------------------------------------------------------------
# 01/12/2024        Michael C Song      File created, rewritten for ascript
####################################################################################################################################
...
class AScriptWriter(MeshWriter):
    """Writes g-code to an AeroScript file.
    ...
    def write(self, stream, nodes, mode = MeshWriter.OutputMode.TextMode):
        ...
        if mode != MeshWriter.OutputMode.TextMode:
            Logger.log("e", "AScriptWriter does not support non-text mode.")
            self.setInformation(catalog.i18nc("@error:not supported", "AScriptWriter does not support non-text mode."))
            return False
        ...
        if gcode_list is not None:
            ...
            for gcode in gcode_list:
                if gcode[:len(self._setting_keyword)] == self._setting_keyword:
                    has_settings = True
                stream.write(gcode)
            # The following is disabled to remove settings footer
    ...
    def _serialiseSettings(self, stack):
        ...
        prefix = self._setting_keyword + str(AScriptWriter.version) + " "  # The prefix to put before each line.
        ...
        pattern = re.compile("|".join(AScriptWriter.escape_characters.keys()))

        # Perform the replacement with a regular expression.
        escaped_string = pattern.sub(lambda m: AScriptWriter.escape_characters[re.escape(m.group(0))], json_string)
